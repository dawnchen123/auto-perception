...
m_CurrentPos = PlannerHNS::WayPoint(0, 0, 0, tf::getYaw(tf::Quaternion(0, 0, 0, 1)));
  ...
  for(unsigned int i = 0; i < m_ObstacleTracking.m_InterestRegions.size(); i++)
  {
    ...
    circle_mkrs.header.frame_id = "velodyne";
  ...
  for(size_t i=0; i<m_DetectedPolygonsAllMarkers.markers.size();i++){
    m_DetectedPolygonsAllMarkers.markers[i].header.frame_id = "velodyne";
  }
  pub_DetectedPolygonsRviz.publish(m_DetectedPolygonsAllMarkers);

  jsk_recognition_msgs::BoundingBoxArray boxes_array;
  boxes_array.header.frame_id = "velodyne";
